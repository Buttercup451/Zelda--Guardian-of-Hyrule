...
function game:set_dialog_style(style)
  ...
  if style == "box" then
    ...
    dialog_box.dialog_surface:set_opacity(255)
  end
end
...
function dialog_box:on_started()
  ...
  local x = camera_width / 2 - 120
  local y = top and 32 or (camera_height - 75)
  ...
  self.question_dst_position = { x = x + 18, y = y + 36 }
  ...
  elseif command == "up" or command == "down" then
    ...
    if self.selected_answer ~= nil
      ...
      self.question_dst_position.y = self.box_dst_position.y +
          (self.selected_answer == 1 and 36 or 53)
    end
  end
...
function dialog_box:on_draw(dst_surface)
  ...
  if self.selected_answer ~= nil
      and self:is_full()
      and not self:has_more_lines() then
    self.box_img:draw_region(96, 76, 8, 8, self.dialog_surface,
        self.question_dst_position.x, self.question_dst_position.y)
  end

  -- Draw the end message arrow.
  if self:is_full() then
    self.end_lines_sprite:draw(self.dialog_surface, x + 108, y + 63)
  end
